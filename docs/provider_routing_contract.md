...
Retry 不执行 sleep、退避、抖动或网络等待。当前没有 observed Fixture、在线 Transport、
真实网络 Retry/Fallback、Circuit Breaker、限流、缓存或正式 Provider 编排。
