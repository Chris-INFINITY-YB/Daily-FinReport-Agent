...
### 3.6 2026-07-25 独立受控观察记录（P1-04R2）

本记录是继 P1-04R 之后单独授权、单独计数的一次新观察，不是前次请求的重试。该观察
已经结束且失败，不构成 P1-04 完成证据，也不授权继续请求。

| 项目 | 安全记录 |
|---|---|
| 观察日期与时区 | `2026-07-25`，`Asia/Shanghai` |
| 调用库/版本 | AkShare `1.18.46` |
| 函数与证券 | `stock_individual_info_em(symbol="600519", timeout=10.0)` |
| 调用边界 | 逻辑调用 `1`，底层请求 `1`，自动重试 `0`，并发 `0`；禁止重试、循环、替代证券或替代接口 |
| 结果 | 失败；安全错误类别为 `json_decode`，异常类型为 `JSONDecodeError` |
| 数据保留 | 未输出或保存原始响应、Response、响应正文、完整 DataFrame、完整 URL、查询参数、Header、Cookie、Token、代理信息或底层异常正文 |
| Fixture | 未创建 observed Fixture，也未从 synthetic 数据或静态源码补写真实结构 |
| 审计源码 | `akshare/stock/stock_info_em.py`，SHA-256 为 `3264436193901655cccf914560327ceb7fc7dbf919785984da6451ca5ea5d33f` |

联网前的静态审计确认目标函数只含一个 `requests.get` 调用且没有循环、递归、自动重试或
分页；项目外一次性脚本先通过 synthetic 对象验证脱敏逻辑，再以有限超时执行唯一一次
请求。安全计数确认没有发生第二个底层请求。

与 2026-07-18 的失败相同，本次 `JSONDecodeError` 出现在成功形成公开 `item/value`
DataFrame 之前。因此没有观察到可用于 Fixture 的真实行结构、字段名、基本类型、空值或
响应证券身份；本记录也不能从未保留的 HTTP 内容推断限流、阻断、协议变化或其他具体
上游原因。

本次独立失败不改变证据等级：`name` 和 `industry` 仍为 E1，不能升级到 E3；
`exchange`、`currency` 和 `description` 继续没有字段证据。P1-04 保持未完成，现有
